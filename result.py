...
class resultClass:
    def __init__(self, root):
        ...
        # === Title ===
        title = Label(self.root, text="Add Student Results", font=("goudy old style", 20, "bold"), bg="#fe5a1d", fg="white").place(x=10, y=15, width=1180, height=50)
        ...
        # === Buttons ===
        btn_add = Button(self.root, text="Submit", command=self.add, font=("times new roman", 15, "bold"), bg="lightgray", cursor="hand2").place(x=300, y=420, width=120, height=30)
        btn_clear = Button(self.root, text="Clear", command=self.clear, font=("times new roman", 15, "bold"), bg="lightgray", cursor="hand2").place(x=430, y=420, width=120, height=30)

        # === SIDE IMAGE (As seen in Video 1) ===
        # Using the absolute path logic to prevent FileNotFoundError
        self.base_path = os.path.dirname(__file__)
        img_path = os.path.join(self.base_path, "images", "result.jpg") 
        
        self.re_img = Image.open(img_path)
        # Resizing to 500x300 to match the layout in Video 1
        self.re_img = self.re_img.resize((500, 300), Image.LANCZOS)
        self.prepare_re_img = ImageTk.PhotoImage(self.re_img)
        
        self.lbl_re_img = Label(self.root, image=self.prepare_re_img, bd=2, relief=RIDGE)
        self.lbl_re_img.place(x=650, y=100)
